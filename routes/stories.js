...
router.post('/', ensureAuth, async (request, response) => {
    try {
        // we're adding a property called 'user' to the object that came through from the form
...
// lean is converting it from a Mongoose object to a plain JSON object
    } catch(err) {
        console.error(err)
        response.render('error/500')
    }
...
router.delete('/:id', ensureAuth, async (request, response) => {
    try {
        ...
        console.error(err)
        return response.return('error/500')
    }
